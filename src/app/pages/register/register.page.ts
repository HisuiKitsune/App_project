...
import {
  ...
  findStreet,
  findZipCode,
} from 'src/app/utils/address-utils';
...
export class RegisterPage implements OnInit {
  ...
  setAddress(place: any) {
    ...
    addressForm
      ?.get('postalcode')
      ?.setValue(findZipCode(place.address_components));
    ...
    addressForm
      ?.get('dist')
      ?.setValue(findNeighborhood(place.address_components));
  }
  ...
  getForm(): FormGroup {
    this.userFormGroup
      .get('repeatPassword')
      ?.setValidators(this.matchPasswordAndRepeatPassword(this.userFormGroup));
  ...
  async createUser(values: any) {
    ...
    await this.firebaseAuthenticationService
      .register(this.userFormGroup.value as CredentialModel)
      .then(() => this.update());
    this.fireBaseService.updateUser(newUser);
    this.dataServiceService.setLogged(true);
    this.userFormGroup.reset();
    if (newUser) {
      this.dataServiceService.setDisplayName(
        this.auth.currentUser!.displayName!
      );
      this.router
        .navigate(['loader'], { replaceUrl: true })
        .then(() =>
          setTimeout(
